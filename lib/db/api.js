...
dbapi.setDeviceAssignedUser = function(serial, email) {
  return db.run(r.table('devices').get(serial).update({
    assignedUser: email
  }))
}

dbapi.unsetDeviceAssignedUser = function(serial) {
  return db.run(r.table('devices').get(serial).update({
    assignedUser: null
  }))
}

dbapi.setDeviceAdminOwner = function(serial, email) {
  return db.run(r.table('devices').get(serial).update({
    adminOwner: email
  }))
}

dbapi.unsetDeviceAdminOwner = function(serial) {
  return db.run(r.table('devices').get(serial).update({
    adminOwner: null
  }))
}

dbapi.loadDevicesByAssignedUser = function(email) {
