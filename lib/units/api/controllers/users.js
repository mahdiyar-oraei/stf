...
function createUser(req, res) {
  ...
  const password = req.swagger.params.password ? req.swagger.params.password.value : null

  console.log('[API DEBUG] Creating user:', { email, name, hasPassword: !!password, hasLdapConfig: !!req.options.ldap })
  if (req.options.ldap) {
    console.log('[API DEBUG] LDAP config present:', {
      url: req.options.ldap.url,
      bindDn: req.options.ldap.bind.dn,
      searchDn: req.options.ldap.search.dn
    })
  } else {
    console.log('[API DEBUG] NO LDAP configuration found in req.options!')
  }

  // First, create user in STF database
  dbapi.createUser(email, name, req.user.ip).then(function(stats) {
    ...
    // If LDAP is configured and password provided, create user in LDAP too
    if (password && req.options.ldap) {
      const ldaputil = require('../../../util/ldaputil')
      const username = name || email.split('@')[0] // Use name or email prefix as username

      return ldaputil.createUser(
        req.options.ldap,
        username,
        name,
        email,
        ...
        apiutil.respond(res, 201, 'Created (user in STF and LDAP)', {
          user: apiutil.publishUser(stats.changes[0].new_val)
        })
      }).catch(function(err) {
        // Log error but still return success since user was created in STF
        log.warn('User created in STF but failed to create in LDAP: ' + err.message)
        apiutil.respond(res, 201, 'Created (user in STF, LDAP creation failed: ' + err.message + ')', {
          user: apiutil.publishUser(stats.changes[0].new_val)
        })
      })
    }
    else {
      // No password provided or LDAP not configured
