...
function observeUserDevice(req, res) {
  ...
  dbapi.loadDevice(req.user.groups.subscribed, serial)
    .then(function(cursor) {
      cursor.next(function(err, device) {
        ...
        if (!deviceutil.canObserve(device, req.user)) {
          return res.status(403).json({
            success: false
          , description: 'Cannot observe this device. Admin access required and device must be in use by another user.'
          })
        }
    ...
    .catch(function(err) {
      log.error('Failed to observe device "%s": ', serial, err.stack)
      res.status(500).json({
        success: false
...
, observeUserDevice: observeUserDevice
}
