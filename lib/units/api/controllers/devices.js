...
function getGenericDevices(req, res, loadDevices) {
  loadDevices(req.user.groups.subscribed).then(function(devices) {
    // For non-admin users, filter to only show assigned devices
    if (req.user.privilege !== apiutil.ADMIN) {
      ...
      // For admin users, filter to show:
      // - Devices with no adminOwner (visible to all admins)
      // - Devices where adminOwner matches current admin
      devices = devices.filter(function(device) {
        return !device.adminOwner || device.adminOwner === req.user.email
      })
    }
...
function assignDeviceToUser(req, res) {
  const serial = req.swagger.params.serial.value
  const email = req.body.email // Email comes from request body

  if (req.user.privilege !== apiutil.ADMIN) {
    return apiutil.respond(res, 403, 'Forbidden: privileged operation (admin)')
  }
  ...
  dbapi.loadDeviceBySerial(serial)
    .then(function(device) {
      ...
      return dbapi.loadUser(email)
        .then(function(user) {
          if (!user) {
            return apiutil.respond(res, 404, 'User not found')
          }
          return dbapi.setDeviceAssignedUser(serial, email)
            .then(function() {
              return apiutil.respond(res, 200, 'Device assigned successfully', {
                serial: serial,
                assignedUser: email
              })
            })
    ...
    .catch(function(err) {
      apiutil.internalError(res, 'Failed to assign device: ', err.stack)
    })
}

function unassignDeviceFromUser(req, res) {
  const serial = req.swagger.params.serial.value
  ...
  dbapi.loadDeviceBySerial(serial)
    .then(function(device) {
      ...
      return dbapi.unsetDeviceAssignedUser(serial)
        .then(function() {
          return apiutil.respond(res, 200, 'Device unassigned successfully', {
            serial: serial
          })
        })
    })
    .catch(function(err) {
      apiutil.internalError(res, 'Failed to unassign device: ', err.stack)
    })
}

function getDevicesByAssignedUser(req, res) {
  ...
  dbapi.loadDevicesByAssignedUser(email)
    .then(function(devices) {
      return apiutil.respond(res, 200, 'Assigned devices', {
        devices: devices.map(function(device) {
          return apiutil.filterDevice(req, device)
        })
      })
    ...
    .catch(function(err) {
      apiutil.internalError(res, 'Failed to load assigned devices: ', err.stack)
    })
}

function setDeviceAdminOwner(req, res) {
  const serial = req.swagger.params.serial.value
  const email = req.body.email // Admin email to own this device
  ...
  dbapi.loadDeviceBySerial(serial)
      ...
      return dbapi.loadUser(email)
        .then(function(user) {
          ...
          if (user.privilege !== apiutil.ADMIN) {
            return apiutil.respond(res, 400, 'Bad Request: user must be an admin')
          }
          return dbapi.setDeviceAdminOwner(serial, email)
            .then(function() {
              return apiutil.respond(res, 200, 'Device admin owner set successfully', {
                serial: serial,
                adminOwner: email
              })
            })
    ...
    .catch(function(err) {
      apiutil.internalError(res, 'Failed to set device admin owner: ', err.stack)
    })
}

function unsetDeviceAdminOwner(req, res) {
  const serial = req.swagger.params.serial.value
  ...
  dbapi.loadDeviceBySerial(serial)
    .then(function(device) {
      ...
      return dbapi.unsetDeviceAdminOwner(serial)
        .then(function() {
          return apiutil.respond(res, 200, 'Device admin owner removed successfully', {
            serial: serial
          })
        })
    })
    .catch(function(err) {
      apiutil.internalError(res, 'Failed to unset device admin owner: ', err.stack)
    })
}
...
, getDevicesByAssignedUser: getDevicesByAssignedUser
, setDeviceAdminOwner: setDeviceAdminOwner
, unsetDeviceAdminOwner: unsetDeviceAdminOwner
}
