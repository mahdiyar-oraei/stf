...
datautil.applyAssignedUser = function(device, user) {
  // Keep assignedUser field visible to all users (they need to see if device is assigned)
  // The field will be present if set in the database
  device.isAssignedToMe = !!(device.assignedUser && device.assignedUser === user.email)
  return device
}
...
datautil.normalize = function(device, user) {
  ...
  datautil.applyAssignedUser(device, user)
  if (!device.present) {
    device.owner = null
  }
}
